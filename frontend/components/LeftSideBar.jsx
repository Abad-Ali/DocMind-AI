...
const LeftSideBar = () => {
  ...
  return (
    <nav className='fixed top-2 lg:top-0.5 lg:bottom-0.5 z-20 bg-black/90 backdrop-blur-lg border-x-2 border-slate-500/70 flex lg:justify-center w-[90vw] sm:w-[95vw] lg:w-[17rem] px-3 lg:px-0 py-2.5 lg:py-0 rounded-2xl lg:rounded-none backdrop-brightness-200'>
                    ...
                    <div onClick={()=>leftSideBarHandler(item.path)} key={index}>
                        <div className={`flex items-center sm:gap-0.5 lg:gap-2 hover:bg-white/5 duration-300 px-1.5 lg:px-3 py-1 lg:py-1.5  text-white cursor-pointer lg:mx-3 my-0.5 ${isActive ? 'bg-gradient-to-r from-blue-800 to via-black to-blue-800 backdrop-brightness-200 duration-300 border border-slate-500 lg:border-none':'border border-black/1 lg:border-none'} duration-300 ${item.text === 'Profile' ? 'rounded-full lg:rounded-lg py-1.5':'rounded-lg'}`}>
                            ...
                            <span className='hidden sm:inline font-semibold cursor-pointer text-[17px]'>
                              {
                                item.text==="Profile" ? (<div className='lg:ml-1 font-black hidden lg:inline'>@{user.username}</div>):(<div>{item.text}</div>)
                              }
                            </span>
                        </div>
                    </div>
